...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer
{
	...
	public function sendChunk($x, $z, $payload)
	{
		...
		$this->usedChunks[(PHP_INT_SIZE === 8 ? ((($x) & 0xFFFFFFFF) << 32) | (($z) & 0xFFFFFFFF) : ($x) . ":" . ($z))] = true;
		$this->chunkLoadCount++;
	...
	protected function doFirstSpawn()
	{
		...
		if (strlen(trim($ev->getJoinMessage())) > 0) {
			$this->server->broadcastMessage($ev->getJoinMessage(), discord: true);
		}
	...
	public function sleepOn(Vector3 $pos)
	{
		...
		$this->setDataProperty(self::DATA_PLAYER_BED_POSITION, self::DATA_TYPE_POS, [
			$pos->x,
			$pos->y,
			$pos->z
		]);
		...
		if (!$revert and $distanceSquared != 0) {
			...
			if ($this->isSurvival()) {
				if (!$revert and !$this->isSleeping()) {
					if ($diff > 0.0625) {
						$revert = true;
						$this->server->getLogger()->warning($this->getServer()
							->getLanguage()
							->translateString("pocketmine.player.invalidMove", [
								$this->getName()
							]));
					}
				}
	...
	public function handleDataPacket(DataPacket $packet)
	{
		...
		switch ($packet::NETWORK_ID) {
			case ProtocolInfo::LOGIN_PACKET:
				...
				$nbt->lastPlayed = new LongTag(floor(microtime(true) * 1000));
				...
				$this->server->getLogger()->info($this->getServer()
					->getLanguage()
					->translateString("pocketmine.player.logIn", [
			...
			case ProtocolInfo::MOVE_PLAYER_PACKET:
				...
				if ($this->teleportPosition !== null or ($this->forceMovement instanceof Vector3 and (($dist = $newPos->distanceSquared($this->forceMovement)) > 0.1 or $revert))) {
					...
					$packet->yaw = (int) $packet->yaw;
					$packet->pitch = (int) $packet->pitch;
					$packet->yaw %= 360;
					$packet->pitch %= 360;
			...
			case ProtocolInfo::USE_ITEM_PACKET:
				...
				if ($packet->face >= 0 and $packet->face <= 5) { // Use Block, place
                    ...
                    if ($item->canBeActivated()) {
                        $item->onActivate($this->level, $this, null, null, $packet->face, $packet->fx, $packet->fy, $packet->fz);
                    }
					...
					if ($item->getId() === Item::SNOWBALL || $item->getId() === Item::EGG) {
						...
						$snowball->setMotion($snowball->getMotion()
							->multiply($f));
						if ($this->isSurvival()) {
						...
						if ($this->startAction > -1 and $this->getDataFlag(self::DATA_FLAGS, self::DATA_FLAG_ACTION)) {
							if ($this->inventory->getItemInHand()->getId() === Item::BOW) {
								...
								if ($ev->isCancelled()) {
									...
									$ev->getProjectile()->setMotion($ev->getProjectile()
										->getMotion()
										->multiply($ev->getForce()));
									if ($this->isSurvival()) {
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if ($action == 2 && $target instanceof Entity and $this->getGamemode() !== Player::VIEW and $this->isAlive() and $target->isAlive()) {
					if ($target instanceof DroppedItem or $target instanceof Arrow) {
						...
						$this->server->getLogger()->warning($this->getServer()
							->getLanguage()
							->translateString("pocketmine.player.invalidEntity", [
								$this->getName()
							]));
						break;
					}
					...
					case 9: // Eating
						...
						if ($this->getHealth() < $this->getMaxHealth() and isset($items[$slot->getId()])) {
							...
							if ($slot->getId() === Item::MUSHROOM_STEW or $slot->getId() === Item::BEETROOT_SOUP) {
								...
								$this->addEffect(Effect::getEffect(Effect::NAUSEA)->setAmplifier(1)
									->setDuration(15 * 20));
								$this->addEffect(Effect::getEffect(Effect::POISON)->setAmplifier(3)
									->setDuration(60 * 20));
							}
						}
						break;
					...
					foreach (explode("\n", $packet->message) as $message) {
						if (trim($message) != "" and strlen($message) <= 255 and $this->messageCounter-- > 0) {
							...
							if (substr($ev->getMessage(), 0, 1) === "/") { // Command
								...
								if (!$ev->isCancelled()) {
									$this->server->broadcastMessage($this->getServer()
										->getLanguage()
										->translateString($ev->getFormat(), [
											$ev->getPlayer()
												->getDisplayName(),
											$ev->getMessage()
										]), $ev->getRecipients(), true);
								}
							}
						}
					}
	...
	public final function close($message = "", $reason = "generic reason", $notify = true)
	{
		if ($this->connected and !$this->closed) {
			...
			if (isset($ev) and $this->username != "" and $this->spawned !== false and $ev->getQuitMessage() != "") {
				$this->server->broadcastMessage($ev->getQuitMessage(), discord: true);
			}
			...
			$this->server->getLogger()->info($this->getServer()
				->getLanguage()
				->translateString("pocketmine.player.logOut", [
					TextFormat::AQUA . $this->getName() . TextFormat::WHITE,
					$this->ip,
					$this->port,
					$this->getServer()
						->getLanguage()
						->translateString($reason)
				]));
			$this->windows = new \SplObjectStorage();
			$this->windowIndex = [];
			$this->usedChunks = [];
	...
	public function save($async = false)
	{
		...
		if ($this->level instanceof Level) {
			...
			$this->namedtag["lastPlayed"] = new LongTag(floor(microtime(true) * 1000));
	...
	public function kill()
	{
		...
		if ($ev->getDeathMessage() != "") {
			$this->server->broadcast($ev->getDeathMessage(), Server::BROADCAST_CHANNEL_USERS, discord: true);
		}
